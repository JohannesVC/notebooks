...
SELECT
    count(*)
FROM
    olympics_history hist
    LEFT JOIN olympics_history_noc_regions regions USING (noc);

-- To see the difference, use except
SELECT
    hist.games,
    hist.noc as hist_noc,
    regions.noc as regions_noc
FROM
    olympics_history hist
    LEFT JOIN olympics_history_noc_regions regions USING (noc)
EXCEPT
...
FROM
    olympics_history hist
    JOIN olympics_history_noc_regions regions USING (noc)
LIMIT
    10;
...
SELECT DISTINCT
    ...
    FIRST_VALUE (number_of_nations) OVER (
        ORDER BY
            number_of_nations
    ) AS Lowest_nr,
    FIRST_VALUE (games) OVER (
...
SELECT
    (
        ...
        FROM
            tot_countries
        WHERE
            number_of_nations = min_max.min_nations
        LIMIT
            1
    ) AS Lowest,
    min_max.min_nations,
    (
        ...
        FROM
            tot_countries
        WHERE
            number_of_nations = min_max.max_nations
        LIMIT
            1
    ) AS Highest,
    min_max.max_nations
